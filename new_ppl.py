...
def lerp(a, b, t):
    ...
    for l in range(a.shape[1]):
        a_l = a[:, l, :].squeeze()
        b_l = b[:, l, :].squeeze()
        lerped = (a_l + (b_l - a_l) * t).unsqueeze(dim=1)
        data.append(lerped)
    ...
    with torch.no_grad():
        for batch in tqdm(batch_sizes):
            try:
                ...
                if args.space == "w":
                    latent = g.mapping(inputs, None)
                    latent_t0, latent_t1 = latent[::2], latent[1::2]
                    latent_e0 = latent_t0.lerp(latent_t1, lerp_t.unsqueeze(1).unsqueeze(2))
                    latent_e1 = latent_t0.lerp(latent_t1, lerp_t.unsqueeze(1).unsqueeze(2) + args.eps)
                    latent_e = torch.stack([latent_e0, latent_e1], 1).view(*latent.shape)

                image = g.synthesis(latent_e, noise_mode="const", force_fp32=True)
